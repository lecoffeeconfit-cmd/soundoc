...
import {
  ActivityIndicator, Alert, Animated, AppState, FlatList, Modal, PanResponder, Platform, Pressable, Share,
  ScrollView, StyleSheet, Text, TextInput, View, type LayoutChangeEvent,
} from 'react-native';
import { createPlaylist, deleteBookmark, deletePlaylist, findLikelyLargeDocumentDuplicate, getDocumentChunkCount, getDocumentText, initializeDatabase, listBookmarks, listDocumentChapters, listItems, listPlaylists, listQueueIds, removeItem, renamePlaylist, saveBookmark, saveHighlight, saveItem, saveLargeDocumentInfo, saveQueueIds, setPlaylistItemIds } from './src/lib/database';
...
import { cleanText, countWords, detectLanguage, estimateSeconds, formatDuration, removeArticleReferenceNoise, safePublicRedirectUrl, safePublicUrl, segmentSentences, suggestedTitle } from './src/lib/text';
import { sectionsFromText } from './src/lib/documents';
...
import { MINI_PLAYER_TAB_BAR_DOCK_HEIGHT, PremiumBottomTabBar } from './src/components/PremiumBottomTabBar';
...
import { FREE_LOW_ALLOWANCE_SECONDS, formatFreeListeningRemaining } from './src/lib/freeListening';
...
const MINI_PLAYER_COLLAPSED_HEIGHT = 18;
// Keep the card four pixels clear of the measured navbar. Its deliberately
...
function cleanedListeningText(item: LibraryItem) {
  const cleaned = item.cleanedText ?? item.text;
  return item.type === 'article' || item.sourceType === 'url' ? removeArticleReferenceNoise(cleaned) : cleaned;
}

function reviewTextFor(item: LibraryItem) {
  return item.speakableText ?? cleanedListeningText(item);
}
...
function SoundocApp() {
  ...
  const handleTabBarLayout = useCallback((event: LayoutChangeEvent) => {
    ...
    setTabBarTop((current) => current !== null && Math.abs(current - nextTop) < 0.5 ? current : nextTop);
  }, []);
  ...
  useEffect(() => {
    ...
    void AsyncStorage.multiGet(['soundoc.onboarding.complete', 'soundoc.listening.defaults', GOLDEN_PROFILE_STORAGE_KEY, 'soundoc.reduce.effects']).then(([onboarding, defaults, storedGoldenProfile, storedReduceEffects]) => {
      setOnboardingComplete(onboarding[1] === 'true');
      setReduceEffects(storedReduceEffects?.[1] === 'true');
  ...
  const openReviewText = useCallback((item: LibraryItem) => {
    if (item.storageMode === 'chunked') {
      const cleanedText = item.cleanedText ?? getDocumentText(item.id);
      if (!cleanedText.trim()) { Alert.alert('Still preparing text', 'Review Text will be ready as soon as Soundoc has prepared the first section.'); return; }
      setPrepared(null);
      setReviewingItem({ ...item, cleanedText });
      return;
    }
    setPrepared(null);
    setReviewingItem(item);
  }, []);
  const saveReviewedText = useCallback((value: string) => {
    const item = reviewingItem;
    const editedText = cleanText(value);
    if (!item || !editedText) { Alert.alert('Keep some listening text', 'Add or retain a few words for Soundoc to read.'); return; }
    const originalListeningText = cleanedListeningText(item);
    const next: LibraryItem = {
      ...item,
      speakableText: editedText === originalListeningText ? undefined : editedText,
      sections: sectionsFromText(editedText, item.title),
      wordCount: countWords(editedText),
      estimatedDurationSeconds: estimateSeconds(countWords(editedText)),
      completed: false,
      updatedAt: Date.now(),
    };
    if (player.item?.id === next.id) player.replaceListeningItem(next);
    else persist(next);
    setReviewingItem(null);
  }, [persist, player, reviewingItem]);
  ...
  const itemActions = (item: LibraryItem) => Alert.alert(item.title, undefined, [
    ...
    { text: 'Review Text', onPress: () => openReviewText(item) },
  ...
  const miniPlayerDockTop = tabBarTop === null
    ? undefined
    : Math.max(0, tabBarTop - MINI_PLAYER_HEIGHT - MINI_PLAYER_DOCK_GAP);

  return (
    <SafeAreaProvider><SafeAreaView style={styles.app} edges={['top', 'bottom']}>
      <StatusBar style="light" />
      {onboardingComplete === null ? <LoadingScreen /> : <>
      {screen === 'home' && <HomeScreen items={recentItems} readyListeningSeconds={readyListeningSeconds} continueItem={continueItem} queue={queue} miniPlayerVisible={Boolean(player.item)} miniPlayerCollapsed={miniPlayerCollapsed} onOpenQueue={() => setShowQueue(true)} onImport={openImport} onUpload={pickDocument} onPhoto={() => importImageText('library')} onCamera={() => importImageText('camera')} onContinue={() => continueItem && openItem(continueItem, true)} onOpen={openItem} onOpenImportInfo={() => setShowImportCapabilities(true)} />}
      ...
      <PreparedModal prepared={prepared} onClose={() => setPrepared(null)} onPlay={playPrepared} onReviewText={() => prepared && openReviewText(prepared.item)} onRetry={() => { if (prepared?.item.storageMode === 'chunked') void processLargeDocument(prepared.item.id, () => refreshLargeImport(prepared.item.id)); }} onPlayNext={() => { if (prepared) addToQueue(prepared.item, true); setPrepared(null); }} onAddToQueue={() => { if (prepared) addToQueue(prepared.item); setPrepared(null); }} />
      <ReviewTextModal item={reviewingItem} onClose={() => setReviewingItem(null)} onSave={saveReviewedText} />
...
function HomeScreen({ items, readyListeningSeconds, continueItem, queue, miniPlayerVisible, miniPlayerCollapsed, onOpenQueue, onImport, onUpload, onPhoto, onCamera, onContinue, onOpen, onOpenImportInfo }: { items: LibraryItem[]; readyListeningSeconds: number; continueItem?: LibraryItem; queue: LibraryItem[]; miniPlayerVisible: boolean; miniPlayerCollapsed: boolean; onOpenQueue: () => void; onImport: (mode: ImportMode) => void; onUpload: () => void; onPhoto: () => void; onCamera: () => void; onContinue: () => void; onOpen: (item: LibraryItem) => void; onOpenImportInfo: () => void }) {
  const homeBottomInset = miniPlayerVisible ? (miniPlayerCollapsed ? MINI_PLAYER_COLLAPSED_HEIGHT : MINI_PLAYER_HEIGHT) + space.lg : space.lg;
  return <ScrollView contentContainerStyle={[styles.scroll, styles.homeScroll, { paddingBottom: homeBottomInset }]} showsVerticalScrollIndicator={false}>
    <View style={[styles.brandRow, styles.homeBrandRow]}><View><Text style={styles.brandMark}>Soundoc</Text><Text style={styles.eyebrow}>PRIVATE LISTENING</Text></View><View style={[styles.privacy, styles.homePrivacy]}><Text style={styles.privacyIcon}>⌁</Text><Text style={styles.privacyText}>On your iPhone</Text></View></View>
    <Text style={[styles.display, styles.homeDisplay]}>{copy.homeTitle}</Text><Text style={[styles.intro, styles.homeIntro]}>{copy.homeSubtitle}</Text>
    <SubscriptionStatusCard readyListeningSeconds={readyListeningSeconds} />
    <View style={[styles.importGroup, styles.homeImportGroup]}>
      <ImportButton symbol="↗" title="Web Article or Link" description="Articles & direct document links" onPress={() => onImport('link')} primary />
      <ImportButton symbol="⌁" title="PDF & Documents" description="Books, textbooks & large documents" onPress={onUpload} />
      <ImportButton symbol="T" title="Paste Text" description="Long manuscripts welcome" onPress={() => onImport('text')} />
    </View>
    {continueItem && <View style={styles.continueDivider} accessibilityElementsHidden><View style={styles.importDividerRule} /></View>}
    {continueItem ? <><Text style={[styles.sectionTitle, styles.continueTitle]}>Continue listening</Text><Pressable style={[styles.continueCard, styles.homeContinueCard]} onPress={onContinue} accessibilityRole="button" accessibilityLabel={`Continue ${continueItem.title}`}><View style={styles.continueTop}><SourceMark item={continueItem} /><View style={styles.grow}><Text style={styles.cardTitle} numberOfLines={2} ellipsizeMode="tail">{continueItem.title}</Text><Text style={styles.meta}>{continueItem.source || typeLabel(continueItem.type)}</Text></View><Text style={styles.playSmall}>▶</Text></View><Progress value={continueItem.progress} /><Text style={styles.remaining}>{Math.round(continueItem.progress * 100)}% complete · {formatDuration(estimateSeconds(continueItem.wordCount * (1 - continueItem.progress), continueItem.rate)).replace('About ', '')} left</Text></Pressable></> : <EmptyState onPress={() => onImport('text')} />}
    ...
    <Pressable onPress={onOpenImportInfo} style={({ pressed }) => [styles.importInfoLink, styles.homeImportInfoLink, pressed && styles.pressed]} accessibilityRole="button" accessibilityLabel="What can I import?" accessibilityHint="Shows supported file types and how larger reading is prepared"><Text style={styles.importInfoIcon}>ⓘ</Text><Text style={styles.importInfoText}>What can I import?</Text><Text style={styles.importInfoChevron}>›</Text></Pressable>
  </ScrollView>;
}
...
function ImportButton({ symbol, title, description, onPress, primary = false }: { symbol: string; title: string; description: string; onPress: () => void; primary?: boolean }) {
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.importButton, styles.homeImportButton, styles.importRaised, primary && styles.importPrimary, primary && styles.homeImportPrimary, pressed && styles.importButtonPressed]} accessibilityRole="button" accessibilityLabel={title} accessibilityHint={description}>
    <View style={[styles.importIcon, styles.homeImportIcon, primary && styles.importIconPrimary, primary && styles.homeImportIconPrimary]}><Text style={[styles.importSymbol, primary && styles.importSymbolPrimary, primary && styles.homeImportSymbolPrimary]}>{symbol}</Text></View><View style={styles.grow}><Text style={[styles.importTitle, primary && styles.importTitlePrimary, primary && styles.homeImportTitlePrimary]}>{title}</Text><Text style={[styles.importDescription, primary && styles.importDescriptionPrimary, primary && styles.homeImportDescriptionPrimary]}>{description}</Text></View><Text style={[styles.chevron, primary && styles.chevronPrimary, primary && styles.homeImportChevronPrimary]}>›</Text>
  </Pressable>;
}
...
function LibraryScreen({ items, playlists, onShowPlaylists, onOpen, onDelete, onToggleFavorite }: { items: LibraryItem[]; playlists: Playlist[]; onShowPlaylists: () => void; onOpen: (item: LibraryItem, autoplay?: boolean) => void; onDelete: (item: LibraryItem) => void; onToggleFavorite: (item: LibraryItem) => void }) {
  ...
  return <View style={styles.fullScreen}><View style={styles.libraryHeader}><View style={styles.libraryTitleRow}><View style={styles.libraryTitleCopy}><Text style={styles.screenTitle}>Library</Text><Text style={styles.screenSubtitle}>Everything stays on this device.</Text></View><Pressable style={({ pressed }) => [styles.libraryPlaylistButton, pressed && styles.pressed]} onPress={onShowPlaylists} accessibilityLabel={`Open playlists, ${playlists.length} playlists`}><Text style={styles.libraryPlaylistIcon}>☷</Text><Text style={styles.libraryPlaylistLabel}>Playlists</Text></Pressable></View><View style={styles.search}><Text style={styles.searchIcon}>⌕</Text><TextInput style={styles.searchInput} value={query} onChangeText={setQuery} placeholder="Search your listening" placeholderTextColor={colors.textTertiary} accessibilityLabel="Search your library" /></View></View>
    <ScrollView horizontal style={styles.libraryFilterScroll} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.libraryFilters} keyboardShouldPersistTaps="handled">
      {(['all', 'recent', 'progress', 'completed', 'favorite', 'queue'] as const).map((value) => <Pressable key={value} onPress={() => setFilter(value)} style={[styles.libraryFilter, filter === value && styles.libraryFilterSelected]}><Text style={[styles.libraryFilterText, filter === value && styles.libraryFilterTextSelected]}>{value === 'progress' ? 'In progress' : value[0].toUpperCase() + value.slice(1)}</Text></Pressable>)}
    </ScrollView>
...
function PlayerScreen({ player, preferences, reduceMotion, hasSummary, freeListening, onOpenFreePlan, showGoldenFeedback, onGoldenGood, onGoldenNotQuite, onGoldenReason, onDismissGoldenFeedback, onOpenSummary, onOpenLearning, onOpenBookmarks, onBookmark, onHighlight, onUpdateSettings, onOpenModeSelector, onViewSource, onReviewText, onClose, onOpenLibrary, onOpenImport, showControls, setShowControls, showListeningStudio, setShowListeningStudio, showVoicePicker, setShowVoicePicker }: { player: ReturnType<typeof useSpeechPlayer>; preferences: ListeningSettings; reduceMotion: boolean; hasSummary: boolean; freeListening: { remainingSeconds: number; resetLabel: string } | null; onOpenFreePlan: () => void; showGoldenFeedback: boolean; onGoldenGood: () => void; onGoldenNotQuite: () => void; onGoldenReason: (reason: GoldenFeedbackReason) => void; onDismissGoldenFeedback: () => void; onOpenSummary: () => void; onOpenLearning: (tab?: LearningToolTab) => void; onOpenBookmarks: () => void; onBookmark: () => void; onHighlight: (sentenceIndex: number) => void; onUpdateSettings: (settings: Partial<ListeningSettings>) => void; onOpenModeSelector: () => void; onViewSource: () => void; onReviewText: () => void; onClose: () => void; onOpenLibrary: () => void; onOpenImport: () => void; showControls: boolean; setShowControls: (value: boolean) => void; showListeningStudio: boolean; setShowListeningStudio: (value: boolean) => void; showVoicePicker: boolean; setShowVoicePicker: (value: boolean) => void }) {
  ...
  const nowReadingPreview = cleanNowReadingPreview(currentSentence);
  ...
  return <View style={[styles.playerScreen, styles.playerScreenCompact]}>
    <View style={[styles.playerHeader, { flexDirection: 'row', alignItems: 'center', minHeight: 45, marginBottom: 0, paddingTop: 0, gap: 0 }]}>
      <TactileIconButton icon="⌄" label="Close player" size={44} onPress={onClose} />
      <View style={styles.playerHeadText}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, minWidth: 0 }}>
          <Text style={[styles.playerHeadKicker, { flexShrink: 1 }]} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>NOW PLAYING</Text>
          {freeListening && <FreeListeningPlayerPill remainingSeconds={freeListening.remainingSeconds} remainingLabel={formatFreeListeningRemaining(freeListening.remainingSeconds)} resetLabel={freeListening.resetLabel} onPress={onOpenFreePlan} reduceMotion={reduceMotion} />}
        </View>
        <MarqueeText text={item.title} reduceMotion={reduceMotion} containerStyle={[styles.playerTitleViewport, { marginTop: 1 }]} textStyle={[styles.playerTitle, { fontSize: 16, lineHeight: 21 }]} />
      </View>
      <Pressable style={[styles.playerModePill, { width: undefined, maxWidth: 116, minHeight: 40, paddingHorizontal: 8, borderRadius: radius.small }]} onPress={onOpenModeSelector} accessibilityRole="button" accessibilityLabel={`Listening mode, ${modeSummary(preferences, item)}`}>
        <Text style={styles.playerModeKicker}>MODE</Text>
        <Text style={styles.playerModeValue} numberOfLines={1}>{modeLabel}</Text>
      </Pressable>
    </View>
    <ScrollView style={[styles.playerScroll, styles.playerScrollCompact]} contentContainerStyle={[styles.playerContent, styles.playerContentCompact]} showsVerticalScrollIndicator={false}>
      ...
      <Pressable style={({ pressed }) => [styles.summaryEntry, pressed && styles.pressed]} onPress={onReviewText} accessibilityRole="button" accessibilityLabel="Review listening text"><View style={styles.summaryEntryIcon}><Text style={styles.summaryEntryGlyph}>✎</Text></View><View style={styles.grow}><Text style={styles.summaryEntryTitle}>Review Text</Text><Text style={styles.summaryEntryText}>Edit what Soundoc will read aloud</Text></View><Text style={styles.summaryEntryChevron}>›</Text></Pressable>
...
function cleanNowReadingPreview(text: string) {
  const preview = text.trimStart();
  if (!preview) return preview;

  // The compact preview can start at an extracted fragment boundary. Remove only
  // punctuation that cannot naturally open a sentence; the listening text stays intact.
  const commaFragment = preview.match(/^,\s*(?:["“”'’]\s*)?/);
  if (commaFragment) return preview.slice(commaFragment[0].length);

  const quoteCommaFragment = preview.match(/^(?:["“”'’]+\s*,\s*)/);
  if (quoteCommaFragment) return preview.slice(quoteCommaFragment[0].length);

  const loneQuote = preview.match(/^["“”'’]+\s*/);
  if (!loneQuote) return preview;
  const candidate = preview.slice(loneQuote[0].length);
  // Preserve a legitimate opening quotation before a normally capitalized sentence.
  return /^[a-z0-9]/.test(candidate) ? candidate : preview;
}

function FreeListeningPlayerPill({ remainingSeconds, remainingLabel, resetLabel, onPress, reduceMotion }: { remainingSeconds: number; remainingLabel: string; resetLabel: string; onPress: () => void; reduceMotion: boolean }) {
  const scale = useRef(new Animated.Value(1)).current;
  const zeroAttention = useRef(new Animated.Value(0)).current;
  const previousRemainingSeconds = useRef(remainingSeconds);
  const zero = remainingSeconds <= 0;
  const low = !zero && remainingSeconds <= FREE_LOW_ALLOWANCE_SECONDS;
  const animateScale = useCallback((toValue: number) => {
    if (reduceMotion) return;
    Animated.spring(scale, { toValue, speed: 30, bounciness: 0, useNativeDriver: true }).start();
  }, [reduceMotion, scale]);
  useEffect(() => {
    const wasAboveZero = previousRemainingSeconds.current > 0;
    previousRemainingSeconds.current = remainingSeconds;
    zeroAttention.stopAnimation();
    zeroAttention.setValue(0);
    if (!wasAboveZero || !zero || reduceMotion) return;
    Animated.sequence([
      Animated.timing(zeroAttention, { toValue: 1, duration: 260, useNativeDriver: true }),
      Animated.timing(zeroAttention, { toValue: 0, duration: 520, useNativeDriver: true }),
    ]).start();
  }, [reduceMotion, remainingSeconds, zero, zeroAttention]);

  const stateStyle = zero
    ? { backgroundColor: 'rgba(255,113,56,0.13)', borderColor: 'rgba(255,155,108,0.78)', shadowColor: colors.accentPrimary, shadowOpacity: 0.18, shadowRadius: 9, elevation: 4 }
    : low
      ? { backgroundColor: 'rgba(216,180,90,0.12)', borderColor: 'rgba(244,215,124,0.60)', shadowColor: colors.accentPrimary, shadowOpacity: 0.13, shadowRadius: 7, elevation: 3 }
      : { borderColor: 'rgba(216,180,90,0.46)', shadowColor: colors.recommendedGoldBright, shadowOpacity: 0.10, shadowRadius: 6, elevation: 2 };
  const timeStyle = zero ? { color: '#FFD1BD' } : low ? { color: colors.recommendedGoldBright } : null;
  const highlightStyle = zero ? { backgroundColor: 'rgba(255,209,189,0.38)' } : low ? { backgroundColor: 'rgba(244,215,124,0.32)' } : null;

  return <Pressable onPress={onPress} onPressIn={() => animateScale(0.98)} onPressOut={() => animateScale(1)} hitSlop={10} style={[styles.freePlayerHeaderPillHit, { minHeight: 44, flexShrink: 1 }]} accessibilityRole="button" accessibilityLabel={`${remainingLabel} free listening this week. ${resetLabel}.`} accessibilityHint="Opens unlimited listening plans">{({ pressed }) => <Animated.View style={[styles.freePlayerHeaderPill, stateStyle, pressed && styles.freePlayerHeaderPillPressed, { flexShrink: 1, transform: [{ scale }] }]}><Animated.View pointerEvents="none" style={[{ position: 'absolute', left: 1, right: 1, top: 1, bottom: 1, borderRadius: radius.pill, borderWidth: 1, borderColor: 'rgba(255,224,208,0.70)', opacity: zeroAttention }]} /><View pointerEvents="none" style={[styles.freePlayerHeaderPillHighlight, highlightStyle]} /><Text style={styles.freePlayerHeaderLabel}>FREE</Text><View pointerEvents="none" style={styles.freePlayerHeaderSeparator} /><Text style={[styles.freePlayerHeaderTime, timeStyle, { flexShrink: 1 }]} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75}>{remainingLabel}</Text><Text style={[styles.freePlayerHeaderChevron, zero && { color: '#FFD1BD' }]}>›</Text></Animated.View>}</Pressable>;
}
...
const MINI_PLAYER_HIDE_DISTANCE = 96;
const MINI_PLAYER_SWIPE_THRESHOLD = 34;

function MiniPlayer({ item, state, currentSentence, collapsed, dockTop, reduceMotion, onPress, onToggle, onCollapse, onRestore }: { item: LibraryItem; state: string; currentSentence: string; collapsed: boolean; dockTop?: number; reduceMotion: boolean; onPress: () => void; onToggle: () => void; onCollapse: () => void; onRestore: () => void }) {
  const translateY = useRef(new Animated.Value(collapsed ? MINI_PLAYER_HIDE_DISTANCE : 0)).current;
  const gestureActive = useRef(false);
  const animateTo = useCallback((toValue: number, velocity = 0, onComplete?: () => void) => {
    if (reduceMotion) {
      translateY.setValue(toValue);
      onComplete?.();
      return;
    }
    Animated.spring(translateY, { toValue, velocity, stiffness: 280, damping: 28, mass: 0.8, useNativeDriver: true }).start(({ finished }) => {
      if (finished) onComplete?.();
    });
  }, [reduceMotion, translateY]);
  useEffect(() => {
    if (!gestureActive.current) animateTo(collapsed ? MINI_PLAYER_HIDE_DISTANCE : 0);
  }, [animateTo, collapsed]);
  const collapse = useCallback((velocity = 0) => {
    gestureActive.current = false;
    animateTo(MINI_PLAYER_HIDE_DISTANCE, velocity, onCollapse);
  }, [animateTo, onCollapse]);
  const restore = useCallback((velocity = 0) => {
    gestureActive.current = false;
    animateTo(0, velocity, onRestore);
  }, [animateTo, onRestore]);
  const dismissGesture = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: () => false,
    onMoveShouldSetPanResponderCapture: (_, gesture) => !collapsed && gesture.dy > 18 && gesture.dy > Math.abs(gesture.dx) * 1.5,
    onPanResponderGrant: () => { gestureActive.current = true; translateY.stopAnimation(); },
    onPanResponderMove: (_, gesture) => translateY.setValue(Math.max(0, Math.min(MINI_PLAYER_HIDE_DISTANCE, gesture.dy))),
    onPanResponderRelease: (_, gesture) => {
      const intentional = gesture.dy > MINI_PLAYER_SWIPE_THRESHOLD || gesture.vy > 0.8;
      intentional ? collapse(Math.max(0, gesture.vy)) : animateTo(0, Math.min(0, gesture.vy));
      if (!intentional) gestureActive.current = false;
    },
    onPanResponderTerminate: () => { gestureActive.current = false; animateTo(0); },
    onPanResponderTerminationRequest: () => false,
  }), [animateTo, collapse, collapsed, translateY]);
  const restoreGesture = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: () => false,
    onMoveShouldSetPanResponderCapture: (_, gesture) => collapsed && gesture.dy < -8 && -gesture.dy > Math.abs(gesture.dx) * 1.35,
    onPanResponderGrant: () => { gestureActive.current = true; translateY.stopAnimation(); },
    onPanResponderMove: (_, gesture) => translateY.setValue(Math.max(0, Math.min(MINI_PLAYER_HIDE_DISTANCE, MINI_PLAYER_HIDE_DISTANCE + gesture.dy))),
    onPanResponderRelease: (_, gesture) => {
      const intentional = gesture.dy < -MINI_PLAYER_SWIPE_THRESHOLD || gesture.vy < -0.8;
      intentional ? restore(Math.min(0, gesture.vy)) : animateTo(MINI_PLAYER_HIDE_DISTANCE, Math.max(0, gesture.vy));
      if (!intentional) gestureActive.current = false;
    },
    onPanResponderTerminate: () => { gestureActive.current = false; animateTo(MINI_PLAYER_HIDE_DISTANCE); },
    onPanResponderTerminationRequest: () => false,
  }), [animateTo, collapsed, restore, translateY]);
  const opacity = translateY.interpolate({ inputRange: [0, MINI_PLAYER_HIDE_DISTANCE * 0.72, MINI_PLAYER_HIDE_DISTANCE], outputRange: [1, 1, 0], extrapolate: 'clamp' });
  const dockPosition = dockTop === undefined ? { bottom: FALLBACK_MINI_PLAYER_DOCK_BOTTOM } : { top: dockTop, bottom: undefined };
  const collapsedDockPosition = dockTop === undefined
    ? { bottom: FALLBACK_MINI_PLAYER_DOCK_BOTTOM }
    : { top: dockTop + MINI_PLAYER_HEIGHT - MINI_PLAYER_COLLAPSED_HEIGHT, bottom: undefined };
  return <>
    <Animated.View {...dismissGesture.panHandlers} pointerEvents={collapsed ? 'none' : 'auto'} style={[styles.miniPlayer, dockPosition, { opacity, transform: [{ translateY }] }]}>
      <Pressable onPress={onPress} style={styles.miniOpen} accessibilityLabel={`${state === 'playing' ? 'Playing' : 'Paused'}: open player for ${item.title}`} accessibilityHint="Swipe down on the player to collapse it"><SourceMark item={item} /><View style={styles.miniCopy}><View style={styles.miniTitleRow}><View style={[styles.miniStatusDot, state === 'playing' ? styles.miniStatusPlaying : styles.miniStatusPaused]} /><MarqueeText text={item.title} reduceMotion={reduceMotion} containerStyle={styles.miniTitleViewport} textStyle={styles.miniTitle} /></View><Text style={styles.miniSentence} numberOfLines={1} ellipsizeMode="tail">“{currentSentence}”</Text><Progress value={item.progress} compact /></View></Pressable><Pressable onPress={onToggle} style={styles.miniToggle} accessibilityLabel={state === 'playing' ? 'Pause' : 'Play'}><Text style={styles.miniToggleIcon}>{state === 'playing' ? 'Ⅱ' : '▶'}</Text></Pressable>
    </Animated.View>
    {collapsed && <Pressable {...restoreGesture.panHandlers} hitSlop={10} onPress={() => restore()} style={[styles.miniCollapsedHandle, collapsedDockPosition]} accessibilityRole="button" accessibilityLabel="Restore mini player" accessibilityHint="Tap or swipe up to restore the player"><View style={styles.miniCollapsedGrabber} /></Pressable>}
  </>;
}
function TabBar({ screen, onChange, miniPlayer, onLayout }: { screen: Screen; onChange: (screen: Screen) => void; miniPlayer: boolean; onLayout: (event: LayoutChangeEvent) => void }) {
  ...
  return <PremiumBottomTabBar selected={screen} onChange={(id) => onChange(id as Screen)} tabs={tabs} hasMiniPlayer={miniPlayer} onLayout={onLayout} />;
}
...
function ImportModal({ mode, text, link, title, busy, onText, onLink, onTitle, onClose, onPaste, onSubmit }: { mode: ImportMode; text: string; link: string; title: string; busy: boolean; onText: (text: string) => void; onLink: (value: string) => void; onTitle: (value: string) => void; onClose: () => void; onPaste: () => void; onSubmit: () => void }) {
  ...
  const [showLinkImportInfo, setShowLinkImportInfo] = useState(false);
  useEffect(() => {
  ...
  useEffect(() => {
    if (mode !== 'link') setShowLinkImportInfo(false);
  }, [mode]);
  const pageLabel = formatDocumentPages(metrics.pages, false);
  return <Modal visible={mode !== null} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
    <SafeAreaView style={styles.importModal}>
      <View style={styles.modalHeader}><View><Text style={styles.screenTitle}>{mode === 'text' ? 'Paste Text' : 'Web Article or Link'}</Text><Text style={styles.screenSubtitle}>{mode === 'text' ? 'Long manuscripts welcome. Soundoc prepares them in sections when needed.' : 'Paste an article or a direct document link.'}</Text></View><Pressable onPress={onClose}><Text style={styles.closeText}>Cancel</Text></Pressable></View>
      {mode === 'text' ? <>
        <View style={styles.titleInputWrap}><Text style={styles.inputLabel}>TITLE (OPTIONAL)</Text><TextInput value={title} onChangeText={onTitle} placeholder="Add a title" placeholderTextColor={colors.textTertiary} style={styles.titleInput} /></View>
        <TextInput value={text} onChangeText={(value) => { onText(value); if (!title) onTitle(suggestedTitle(value)); }} multiline autoFocus placeholder="Paste or write something here…" placeholderTextColor={colors.textTertiary} style={styles.textEditor} textAlignVertical="top" accessibilityLabel="Text to listen to" />
        ...
        <ImportCapacityHint>Long manuscripts are saved in durable sections so you can resume where you left off.</ImportCapacityHint>
      </> : <>
        <TextInput value={link} onChangeText={onLink} autoCapitalize="none" autoCorrect={false} keyboardType="url" autoFocus placeholder="https://example.com/article-or-document" placeholderTextColor={colors.textTertiary} style={styles.linkInput} accessibilityLabel="Article or document link" />
        <Pressable onPress={() => setShowLinkImportInfo(true)} style={({ pressed }) => [styles.linkImportInfoAction, pressed && styles.pressed]} accessibilityRole="button" accessibilityLabel="How link imports work" accessibilityHint="Explains article and document link imports"><Text style={styles.linkImportInfoIcon}>ⓘ</Text><Text style={styles.linkImportInfoText}>How link imports work</Text><Text style={styles.linkImportInfoChevron}>›</Text></Pressable>
        <Text style={styles.linkImportReassurance}>Public links are fetched directly from their original site.</Text>
      </>}
      <Pressable onPress={onSubmit} disabled={busy} style={[styles.modalPrimary, busy && styles.disabled]}><Text style={styles.modalPrimaryText}>{busy ? 'Preparing…' : 'Prepare to listen'}</Text><Text style={styles.modalPrimaryArrow}>›</Text></Pressable>
    </SafeAreaView>
    <LinkImportInfoModal visible={showLinkImportInfo} onClose={() => setShowLinkImportInfo(false)} />
  </Modal>;
}

function LinkImportInfoModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
    <View style={styles.linkImportInfoOverlay}>
      <Pressable style={styles.linkImportInfoBackdrop} onPress={onClose} accessibilityLabel="Close link import information" />
      <View style={styles.linkImportInfoSheet} accessibilityViewIsModal>
        <View style={styles.linkImportInfoSheetHeader}><Text style={styles.linkImportInfoSheetTitle}>How link imports work</Text><Pressable onPress={onClose} hitSlop={10} accessibilityLabel="Close link import information"><Text style={styles.closeText}>Done</Text></Pressable></View>
        <Text style={styles.linkImportInfoSheetText}>Soundoc can import public article links and direct document links.</Text>
        <Text style={styles.linkImportInfoSheetText}>Public article pages are cleaned for listening, while direct PDF, EPUB, DOCX, and text links are imported as documents.</Text>
        <Text style={styles.linkImportInfoSheetText}>Pages that require a sign-in or active subscription may not be accessible directly. If that happens, you can paste the article text instead.</Text>
      </View>
    </View>
  </Modal>;
}
...
function ReviewTextModal({ item, onClose, onSave }: { item: LibraryItem | null; onClose: () => void; onSave: (text: string) => void }) {
  const [draft, setDraft] = useState('');
  const [changed, setChanged] = useState(false);
  const cleanedOriginal = item ? cleanedListeningText(item) : '';
  useEffect(() => {
    if (!item) return;
    setDraft(reviewTextFor(item));
    setChanged(false);
  }, [item?.id]);
  if (!item) return null;
  const close = () => {
    if (!changed) { onClose(); return; }
    Alert.alert('Discard edits?', 'Your changes have not been saved to the listening version.', [{ text: 'Keep editing', style: 'cancel' }, { text: 'Discard', style: 'destructive', onPress: onClose }]);
  };
  const reset = () => {
    if (!changed && draft === cleanedOriginal) return;
    Alert.alert('Reset listening text?', 'This will replace your edits with Soundoc’s cleaned listening version.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Reset', style: 'destructive', onPress: () => { setDraft(cleanedOriginal); setChanged(true); } }]);
  };
  return <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={close}>
    <SafeAreaView style={styles.reviewModal}>
      <View style={styles.modalHeader}>
        <View style={styles.reviewHeaderCopy}><Text style={styles.screenTitle}>Review Listening Text</Text><Text style={styles.screenSubtitle}>Edit or remove anything you don’t want Soundoc to read. Your original import stays unchanged.</Text></View>
        <Pressable onPress={close} style={styles.reviewHeaderAction} accessibilityRole="button" accessibilityLabel="Close Review Text"><Text style={styles.closeText}>Done</Text></Pressable>
      </View>
      <View style={styles.reviewDocumentMeta}><Text style={styles.reviewDocumentLabel}>LISTENING VERSION</Text><Text style={styles.reviewDocumentTitle} numberOfLines={1}>{item.title}</Text><Text style={styles.reviewSavedState}>{item.speakableText ? 'Custom listening text' : 'Soundoc cleaned text'}</Text></View>
      <TextInput value={draft} onChangeText={(value) => { setDraft(value); if (!changed) setChanged(true); }} multiline autoCorrect style={styles.reviewEditor} textAlignVertical="top" accessibilityLabel="Listening text editor" accessibilityHint="Edit the exact text Soundoc will read aloud" />
      <View style={styles.reviewFooter}>
        <Pressable onPress={reset} style={styles.reviewReset} accessibilityRole="button" accessibilityLabel="Reset to cleaned original"><Text style={styles.reviewResetText}>Reset to Cleaned Original</Text></Pressable>
        <Pressable onPress={() => onSave(draft)} style={styles.reviewSave} accessibilityRole="button" accessibilityLabel="Save listening text"><Text style={styles.reviewSaveText}>Save Changes</Text></Pressable>
      </View>
    </SafeAreaView>
  </Modal>;
}

function PreparedModal({ prepared, onClose, onPlay, onReviewText, onRetry, onPlayNext, onAddToQueue }: { prepared: Prepared; onClose: () => void; onPlay: () => void; onReviewText: () => void; onRetry: () => void; onPlayNext: () => void; onAddToQueue: () => void }) {
...
function MarqueeText({ text, containerStyle, textStyle, reduceMotion = false }: { text: string; containerStyle?: object; textStyle?: object; reduceMotion?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (reduceMotion || !appActive || contentWidth <= viewportWidth || !viewportWidth) return;
    const distance = contentWidth - viewportWidth + 14;
    const animation = Animated.loop(Animated.sequence([Animated.delay(1700), Animated.timing(translateX, { toValue: -distance, duration: Math.max(4200, distance * 42), useNativeDriver: true }), Animated.delay(1000), Animated.timing(translateX, { toValue: 0, duration: 650, useNativeDriver: true }), Animated.delay(1700)]));
    animation.start();
    return () => animation.stop();
  }, [appActive, contentWidth, reduceMotion, translateX, viewportWidth]);
...
const styles = StyleSheet.create({
  ...
  importModal: { flex: 1, padding: space.xl, backgroundColor: colors.backgroundPrimary }, modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: space.xl }, summaryModalHeader: { paddingTop: space.md }, titleInputWrap: { marginBottom: space.sm }, inputLabel: { ...type.caption, color: colors.textTertiary, letterSpacing: 0.7, marginBottom: 5 }, titleInput: { ...type.body, height: 48, paddingHorizontal: space.sm, backgroundColor: colors.surfacePrimary, borderRadius: radius.small, borderWidth: 1, borderColor: colors.borderSubtle, color: colors.textPrimary }, textEditor: { ...type.body, color: colors.textPrimary, flex: 1, minHeight: 210, padding: space.md, backgroundColor: colors.surfacePrimary, borderRadius: radius.medium, borderWidth: 1, borderColor: colors.borderSubtle }, editorFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: space.sm }, clipboard: { paddingVertical: space.xs }, clipboardText: { ...type.label, color: colors.accentPrimary }, wordMeta: { ...type.caption, color: colors.textSecondary, paddingHorizontal: 8, paddingVertical: 5, borderRadius: radius.small, backgroundColor: colors.surfaceInset, borderWidth: 1, borderTopColor: 'rgba(0,0,0,0.45)', borderBottomColor: 'rgba(255,255,255,0.06)' }, linkInput: { ...type.body, height: 54, paddingHorizontal: space.md, color: colors.textPrimary, backgroundColor: colors.surfacePrimary, borderRadius: radius.medium, borderWidth: 1, borderColor: colors.borderSubtle }, linkImportInfoAction: { minHeight: 44, marginTop: space.sm, paddingHorizontal: space.xs, flexDirection: 'row', alignItems: 'center', gap: 6, alignSelf: 'flex-start' }, linkImportInfoIcon: { color: colors.textTertiary, fontSize: 15 }, linkImportInfoText: { ...type.caption, color: colors.textSecondary }, linkImportInfoChevron: { color: colors.textTertiary, fontSize: 19, lineHeight: 20 }, linkImportReassurance: { ...type.caption, color: colors.textTertiary, lineHeight: 17, marginTop: 1, paddingHorizontal: space.xs }, linkImportInfoOverlay: { flex: 1, justifyContent: 'flex-end' }, linkImportInfoBackdrop: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(8,10,12,0.58)' }, linkImportInfoSheet: { marginHorizontal: space.md, marginBottom: space.lg, padding: space.lg, borderRadius: radius.large, backgroundColor: colors.surfacePrimary, borderWidth: 1, borderTopColor: 'rgba(255,255,255,0.12)', borderBottomColor: 'rgba(0,0,0,0.72)', shadowColor: '#000000', shadowOpacity: 0.34, shadowOffset: { width: 0, height: 10 }, shadowRadius: 22, elevation: 12 }, linkImportInfoSheetHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: space.sm, marginBottom: space.sm }, linkImportInfoSheetTitle: { ...type.heading, color: colors.textPrimary }, linkImportInfoSheetText: { ...type.caption, color: colors.textSecondary, lineHeight: 19, marginTop: space.xs }, modalPrimary: { height: 56, marginTop: space.xl, backgroundColor: colors.accentPrimary, borderRadius: radius.medium, flexDirection: 'row', paddingHorizontal: space.lg, alignItems: 'center', justifyContent: 'space-between' }, disabled: { opacity: 0.5 }, modalPrimaryText: { ...type.heading, color: '#FFFFFF' }, modalPrimaryArrow: { color: '#FFFFFF', fontSize: 28 }, loadingOverlay: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(23,23,33,0.72)', alignItems: 'center', justifyContent: 'center', gap: space.md }, loadingText: { ...type.label, color: '#FFFFFF' },
  ...
  libraryFilterScroll: { height: 34, flexGrow: 0, flexShrink: 0 }, libraryFilters: { paddingHorizontal: space.xl, flexDirection: 'row', alignItems: 'center', gap: space.xs }, libraryFilter: { minHeight: 34, paddingHorizontal: space.sm, borderRadius: radius.pill, backgroundColor: colors.surfaceInset, borderWidth: 1, borderColor: colors.borderSubtle, alignItems: 'center', justifyContent: 'center' }, libraryFilterSelected: { backgroundColor: colors.accentSoft, borderColor: colors.accentPrimary }, libraryFilterText: { ...type.caption, color: colors.textTertiary }, libraryFilterTextSelected: { color: colors.accentPrimary }, librarySorts: { marginTop: space.lg, paddingHorizontal: space.xl, paddingBottom: space.sm, flexDirection: 'row', gap: space.md }, librarySortText: { ...type.caption, color: colors.textTertiary }, librarySortSelected: { color: colors.accentPrimary }, favoriteMark: { color: colors.accentPrimary, fontSize: 19, marginRight: 2 },
  ...
  artworkTopCompactLifted: { marginTop: space.xxs },
  homeScroll: { paddingTop: space.lg },
  homeBrandRow: { marginTop: 0, marginBottom: 34 },
  homePrivacy: { paddingHorizontal: 9, paddingVertical: 5, backgroundColor: 'rgba(255,113,56,0.08)', borderWidth: 1, borderColor: 'rgba(255,183,137,0.18)' },
  homeDisplay: { fontSize: 36, lineHeight: 42 },
  homeIntro: { marginTop: 6 },
  homeImportGroup: { marginTop: space.xl, gap: 10 },
  homeContinueCard: { marginTop: space.md },
  homeImportButton: { minHeight: 80, paddingVertical: 14, paddingHorizontal: space.md, backgroundColor: colors.surfacePrimary, borderTopColor: 'rgba(255,255,255,0.10)', borderBottomColor: 'rgba(0,0,0,0.66)', shadowColor: '#000000', shadowOpacity: 0.16, shadowOffset: { width: 0, height: 5 }, shadowRadius: 10, elevation: 3 },
  homeImportPrimary: { backgroundColor: '#202326', borderColor: 'rgba(255,113,56,0.38)', borderTopColor: 'rgba(255,183,137,0.42)', borderBottomColor: 'rgba(255,113,56,0.38)', shadowColor: colors.accentPrimary, shadowOpacity: 0.14, shadowOffset: { width: 0, height: 6 }, shadowRadius: 12, elevation: 4 },
  homeImportIcon: { width: 44, height: 44 },
  homeImportIconPrimary: { backgroundColor: 'rgba(255,113,56,0.16)', borderWidth: 1, borderColor: 'rgba(255,183,137,0.24)' },
  homeImportSymbolPrimary: { color: colors.accentPrimary },
  homeImportTitlePrimary: { color: colors.textPrimary },
  homeImportDescriptionPrimary: { color: colors.textSecondary },
  homeImportChevronPrimary: { color: colors.accentPrimary },
  homeImportInfoLink: { marginTop: space.xl, alignSelf: 'center' },
  reviewModal: { flex: 1, paddingHorizontal: space.xl, paddingTop: space.md, paddingBottom: space.md, backgroundColor: colors.backgroundPrimary },
  reviewHeaderCopy: { flex: 1, paddingRight: space.sm },
  reviewHeaderAction: { minHeight: 44, minWidth: 48, alignItems: 'flex-end', justifyContent: 'center' },
  reviewDocumentMeta: { marginBottom: space.md, padding: space.md, borderRadius: radius.large, backgroundColor: colors.surfaceElevated, borderWidth: 1, borderTopColor: 'rgba(255,255,255,0.10)', borderBottomColor: 'rgba(0,0,0,0.68)' },
  reviewDocumentLabel: { ...type.caption, color: colors.accentPrimary, letterSpacing: 0.9 },
  reviewDocumentTitle: { ...type.heading, color: colors.textPrimary, marginTop: 3 },
  reviewSavedState: { ...type.caption, color: colors.textSecondary, marginTop: 3 },
  reviewEditor: { flex: 1, minHeight: 260, padding: space.md, borderRadius: radius.large, backgroundColor: colors.surfacePrimary, borderWidth: 1, borderColor: colors.borderSubtle, color: colors.textPrimary, fontSize: 17, lineHeight: 27 },
  reviewFooter: { flexDirection: 'row', gap: space.sm, paddingTop: space.md },
  reviewReset: { flex: 1, minHeight: 50, paddingHorizontal: space.sm, borderRadius: radius.medium, backgroundColor: colors.surfaceInset, borderWidth: 1, borderTopColor: 'rgba(255,255,255,0.08)', borderBottomColor: 'rgba(0,0,0,0.62)', alignItems: 'center', justifyContent: 'center' },
  reviewResetText: { ...type.caption, color: colors.textSecondary, textAlign: 'center' },
  reviewSave: { flex: 1, minHeight: 50, paddingHorizontal: space.sm, borderRadius: radius.medium, backgroundColor: colors.accentPrimary, alignItems: 'center', justifyContent: 'center' },
  reviewSaveText: { ...type.label, color: '#FFFFFF' },
  preparedReview: { minHeight: 44, alignSelf: 'stretch', alignItems: 'center', justifyContent: 'center', marginTop: space.xs },
  preparedReviewText: { ...type.label, color: colors.accentPrimary },
});
