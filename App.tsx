...
function PlayerScreen({ player, preferences, reduceMotion, hasSummary, freeListening, onOpenFreePlan, showGoldenFeedback, onGoldenGood, onGoldenNotQuite, onGoldenReason, onDismissGoldenFeedback, onOpenSummary, onOpenLearning, onOpenBookmarks, onBookmark, onHighlight, onUpdateSettings, onOpenModeSelector, onClose, onOpenLibrary, onOpenImport, showControls, setShowControls, showListeningStudio, setShowListeningStudio, showVoicePicker, setShowVoicePicker }: { player: ReturnType<typeof useSpeechPlayer>; preferences: ListeningSettings; reduceMotion: boolean; hasSummary: boolean; freeListening: { remainingSeconds: number; resetLabel: string; critical: boolean } | null; onOpenFreePlan: () => void; showGoldenFeedback: boolean; onGoldenGood: () => void; onGoldenNotQuite: () => void; onGoldenReason: (reason: GoldenFeedbackReason) => void; onDismissGoldenFeedback: () => void; onOpenSummary: () => void; onOpenLearning: (tab?: LearningToolTab) => void; onOpenBookmarks: () => void; onBookmark: () => void; onHighlight: (sentenceIndex: number) => void; onUpdateSettings: (settings: Partial<ListeningSettings>) => void; onOpenModeSelector: () => void; onClose: () => void; onOpenLibrary: () => void; onOpenImport: () => void; showControls: boolean; setShowControls: (value: boolean) => void; showListeningStudio: boolean; setShowListeningStudio: (value: boolean) => void; showVoicePicker: boolean; setShowVoicePicker: (value: boolean) => void }) {
  ...
  const seekTo = (percentage: number) => { const target = Math.round(Math.max(0, player.sentences.length - 1) * (percentage / 100)); player.jump(target - item.sentenceIndex); };
  ...
  return <View style={[styles.playerScreen, styles.playerScreenCompact]}>
    ...
    <ScrollView style={[styles.playerScroll, styles.playerScrollCompact]} contentContainerStyle={[styles.playerContent, styles.playerContentCompact]} showsVerticalScrollIndicator={false}>
      <RaisedGraphiteCard style={[styles.playerArtwork, styles.playerArtworkCompact]}>
        ...
        <View style={[styles.artworkMark, styles.artworkMarkCompact]}><Text style={[styles.artworkGlyph, styles.artworkGlyphCompact]}>{item.type === 'article' ? '↗' : item.type === 'document' ? '▤' : 'T'}</Text><Text style={styles.artworkWave}>⌁</Text></View>
        ...
        <SegmentedControlDial value={item.progress * 100} onChange={seekTo} compact />
        <Text style={styles.dialHint}>Tap a segment to jump through the document</Text>
      </RaisedGraphiteCard>
      <View style={[styles.progressArea, styles.progressAreaCompact]}><View style={styles.progressLabels}><Text style={styles.progressMeta}>{item.storageMode === 'chunked' && item.processingStatus && item.processingStatus !== 'ready' ? `Preparing · ${item.processedUnits?.toLocaleString() ?? 0} sections` : `${Math.round(item.progress * 100)}% complete`}</Text><Text style={styles.progressMeta}>{formatDuration(remainingListeningSeconds(item)).replace('About ', '')} left</Text></View><Progress value={item.progress} onChange={seekTo} /></View>
      {freeListening && <Pressable style={({ pressed }) => [styles.freePlayerIndicator, freeListening.critical && styles.freePlayerIndicatorCritical, pressed && styles.pressed]} onPress={onOpenFreePlan} accessibilityRole="button" accessibilityLabel={`${formatFreeListeningRemaining(freeListening.remainingSeconds)} free listening this week. ${freeListening.resetLabel}.`} accessibilityHint="Opens unlimited listening plans"><View><Text style={styles.freePlayerKicker}>FREE LISTENING</Text><Text style={styles.freePlayerValue}>{formatFreeListeningRemaining(freeListening.remainingSeconds)} this week</Text></View><Text style={styles.freePlayerReset}>{freeListening.resetLabel} ›</Text></Pressable>}
      <View style={[styles.mainControls, styles.mainControlsCompact]}><TactileIconButton icon="‹‹" label="Previous sentence" size={50} onPress={() => player.jump(-1)} /><Pressable style={({ pressed }) => [styles.playerPlayButton, styles.playerPlayButtonCompact, pressed && styles.playerPlayPressed]} onPress={() => player.state === 'playing' ? player.pause() : player.play()} accessibilityRole="button" accessibilityLabel={player.state === 'playing' ? 'Pause' : 'Play'}><Text style={styles.playIcon}>{player.state === 'playing' ? 'Ⅱ' : '▶'}</Text></Pressable><TactileIconButton icon="››" label="Next sentence" size={50} onPress={() => player.jump(1)} /></View>
...
const styles = StyleSheet.create({
  ...
  miniPlayer: { position: 'absolute', bottom: 69, left: space.md, right: space.md, height: 76, borderRadius: radius.medium, paddingLeft: 12, paddingRight: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.glassTint, borderWidth: 1, borderColor: colors.borderSubtle, shadowColor: '#121218', shadowOpacity: 0.13, shadowOffset: { width: 0, height: 7 }, shadowRadius: 18, elevation: 6 }, miniOpen: { flex: 1, flexDirection: 'row', gap: space.sm, alignItems: 'center' }, miniTitle: { ...type.label, color: colors.textPrimary, fontSize: 15, lineHeight: 20, fontWeight: '700' }, miniToggle: { width: 48, height: 48, borderRadius: radius.pill, backgroundColor: colors.accentPrimary, alignItems: 'center', justifyContent: 'center', marginLeft: space.sm }, miniToggleIcon: { color: '#FFFFFF', fontSize: 18, paddingLeft: 1 }, tabBar: { height: 72, backgroundColor: 'rgba(255,255,255,0.96)', borderTopWidth: 1, borderColor: colors.borderSubtle, flexDirection: 'row', paddingBottom: Platform.OS === 'ios' ? 6 : 0 }, tabBarWithPlayer: { paddingTop: 0 }, tab: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 1 }, tabIcon: { color: colors.textTertiary, fontSize: 19 }, tabLabel: { ...type.caption, color: colors.textTertiary }, tabSelected: { color: colors.accentPrimary },
  ...
  libraryTitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: space.sm }, libraryPlaylistButton: { minWidth: 168, minHeight: 52, paddingHorizontal: space.md, borderRadius: radius.medium, backgroundColor: colors.surfaceElevated, borderWidth: 1, borderTopColor: 'rgba(255,113,56,0.28)', borderBottomColor: 'rgba(255,113,56,0.18)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, flexShrink: 0, shadowColor: '#000', shadowOpacity: 0.25, shadowOffset: { width: 0, height: 4 }, shadowRadius: 7, elevation: 3 }, libraryPlaylistIcon: { width: 20, color: colors.accentPrimary, fontSize: 19, textAlign: 'center', textShadowColor: 'rgba(255,113,56,0.30)', textShadowRadius: 4, textShadowOffset: { width: 0, height: 0 } }, libraryPlaylistLabel: { ...type.label, color: colors.textPrimary },
