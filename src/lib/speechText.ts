...
export type SpeakableChunk = { text: string; pauseAfterMs: number; paragraphIndex: number; sentenceIndex: number; characterOffset: number };
...
export function applyReadingRules(text: string, preferences: Pick<SpeechPreferences, 'skipHeadings' | 'skipUrls' | 'skipCitations' | 'skipConsecutiveDuplicates' | 'skipLongNumbersAndCodes'>) {
  const numberCodeFiltered = filterLongNumbersAndCodes(text, { enabled: preferences.skipLongNumbersAndCodes !== false }).spokenText;
  const paragraphs = cleanText(numberCodeFiltered).split(/\n{2,}/).map((paragraph) => paragraph.trim()).filter(Boolean);
  let previousKey = '';
  const kept: string[] = [];
...
export function processSpeechText(text: string, preferences: SpeechPreferences, language = 'en-US'): SpeakableChunk[] {
  ...
  paragraphs.forEach((paragraph, paragraphIndex) => {
    const sentences = segmentSentences(paragraph, language);
    sentences.forEach((sentence, sentenceIndex) => {
      for (let offset = 0; offset < sentence.length; offset += MAX_CHUNK_LENGTH) {
        ...
        if (part) chunks.push({ text: part, pauseAfterMs: 0, paragraphIndex, sentenceIndex, characterOffset: offset });
      }
    });
  });
  chunks.forEach((chunk, index) => {
    const next = chunks[index + 1];
    chunk.pauseAfterMs = next && next.paragraphIndex !== chunk.paragraphIndex ? preferences.paragraphPauseMs : next && next.sentenceIndex !== chunk.sentenceIndex ? preferences.sentencePauseMs : 0;
  });
  return chunks;
}
