...
export function SubscriptionSettingsSection() {
  ...
  const status = subscription.isTrialing ? 'Free trial' : subscription.isPro ? 'Active' : 'Soundoc Pro';
  ...
  return <View style={styles.section}>
    ...
    <View style={styles.card}>
      <Pressable onPress={subscription.openPaywall} style={({ pressed }) => [styles.summary, pressed && styles.pressed]} accessibilityRole="button" accessibilityLabel={`${status}. ${detail}. View Soundoc Pro plans.`}>
        <View style={[styles.icon, subscription.isPro && styles.iconActive]}><Text style={styles.iconText}>✦</Text></View><View style={styles.copy}><Text style={styles.label}>{status}</Text><Text style={styles.detail}>{detail}</Text></View><Text style={styles.chevron}>›</Text>
      </Pressable>
      <View style={styles.actions}>
        ...
        <Pressable onPress={subscription.openPaywall} style={({ pressed }) => [styles.action, styles.actionBorder, pressed && styles.pressed]} accessibilityRole="button" accessibilityLabel="View Soundoc Pro plans"><Text style={styles.actionText}>{subscription.isPro ? 'View Plans' : 'View Pro'}</Text></Pressable>
      </View>
    </View>
  </View>;
}
