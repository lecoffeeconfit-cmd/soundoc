 ...
 */
export function SegmentedControlDial({ value, onChange, label = 'Document navigation', compact = false, markers = [], jumpTargets, context, totalDurationSeconds, reduceMotion = false }: Props) {
  const size = compact ? 188 : 226;
  const trackRadius = compact ? 77 : 94;
  const centerDiameter = compact ? 92 : 112;
  ...
  return <View style={[styles.wrap, compact && styles.wrapCompact]}>
    ...
    <View style={styles.contextRow}>
      <Text style={styles.contextLabel} numberOfLines={1}>{context?.currentLabel ?? context?.preparingLabel ?? 'Continuous navigation'}</Text>
    </View>
    ...
    <Modal visible={jumpToVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setJumpToVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  contextRow: { minHeight: 20, marginTop: space.md, paddingHorizontal: space.md, justifyContent: 'center', alignItems: 'center' }, contextLabel: { ...type.caption, color: colors.textSecondary, textAlign: 'center', lineHeight: 17 },
